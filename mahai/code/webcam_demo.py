...
def inference(model):
    ...
    while True:
        ...
        if not results:
            ...
            sign_spotter_time = time.time()

            def check_results(tmp_window):
                if not model.predict(tmp_window):
                    return False
                return True

            # print(np.array(cur_windows).shape)
            l, r = 20, len(cur_windows)
            while r - l > 1:
                m = (l + r) // 2
                tmp_frame_matrix = np.concatenate(
                    ((np.zeros((len(cur_windows) - m, len(cur_windows[0]), len(cur_windows[0][0]), 3)),
                      np.array(cur_windows[-m:]))))
                # print(len(cur_windows))
                # print(tmp_frame_matrix.shape)
                if check_results(tmp_frame_matrix):
                    r = m
                else:
                    l = m

            print(results, l, "frames is needed", l / fps, "dure sec", time.time() - sign_spotter_time, "work sec")

            out = cv2.VideoWriter("output.mp4", cv2.VideoWriter_fourcc(*'mp4v'), fps, (224, 224))
            for frame in cur_windows[-l:]:
...
def main():
    ...
    global frame_width, frame_height, fps
