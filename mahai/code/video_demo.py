...
def split_list_into_batches(input_list, frame_start_timestamps, stride, window_size=32):
    """
    ...
    if len(input_list) <= window_size:
        return [input_list], frame_start_timestamps
    else:
        batches = []
        timestamps_out = []
        for i in range(0, len(input_list) - window_size - 1, stride):
            batches.append(input_list[i:i + window_size])
            timestamps_out.append([frame_start_timestamps[i], frame_start_timestamps[i + window_size]])
        return batches, timestamps_out
...
def process_video(video_path, config_path, stride=1, window_size=32):
    ...
    for i in range(frame_count):
        try:
            ...
            frames.append(frame)
            frame_start_timestamps.append(cap.get(cv2.CAP_PROP_POS_MSEC))
        except cv2.error as e:
            continue
    ...
    frames, batch_timestamps = split_list_into_batches(
        frames, frame_start_timestamps, stride, window_size=window_size)

    result = []
    for i, batch in tqdm(enumerate(frames), total=len(frames)):
        cur_word = process_frames(batch, model)
        if cur_word is not None:
            if len(result) == 0 or result[-1][2] != cur_word:
                result.append([batch_timestamps[i][0], batch_timestamps[i][1], cur_word])
            elif result[-1][2] == cur_word:
                result[-1][1] = batch_timestamps[i][1]

    output_json = []
    for word_meta in result:
        output_json.append({
            "timestamp_start": word_meta[0] / 1000,
            "timestamp_end": word_meta[1] / 1000,
            "translation": word_meta[2]
        })
    return output_json
